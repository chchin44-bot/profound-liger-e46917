...
const run = (ind, pbrHist) => p.evaluate(([ind,pbrHist])=>{
  ...
  return { title: i<0 ? '' : txt.slice(i, i+120), whole: txt,
           blocked: s.data.targets.longBlocked || null };
}, [ind,pbrHist]);
...
console.log('金融股標題：', fin.title, '| blocked=', fin.blocked);
/* 先驗前置條件真的成立。少了這一步，regexp 剛好找不到也會變成假綠。 */
...
/* v93h：第四層的證據鏈在 blocked 的情況下也必須出現，不能安靜地少一塊。 */
T(/這次的判定可信度/.test(fin.whole), '金融股也看得到第四層：可信度與原因');

const gen = await run('半導體業', null);
console.log('一般股標題：', gen.title, '| blocked=', gen.blocked);
